package com.baokaicong.android.bmusic.consts;

public class Strings {
    public static final String NET_ERROR="网络貌似开小差啦~~";

    public static final String UPGLIDE_LOAD="上滑加载更多哟~~";

    public static final String LOAD_END="已经加载完啦~~";

    public static final String DATA_LOAD_ERROR="数据貌似被干掉了~~";

    public static final String DATA_EMPTY="数据被吃得光光的~~";

    public static final String DATA_LOADING="数据正在吃命加载中~~";

    public static final String MENU_LOADING="歌单吃命同步中,小主喝口水再试试~~";

    public static final String MENU_ADD_MUSIC_SUCCESS="它被你纳入怀中啦~~";

    public static final String MENU_ADD_MUSIC_FAIL="小主的请求貌似被拒绝了~~";

    public static final String MENU_DROP_MUSIC_SUCCESS="它已回到那诗与远方~~";

    public static final String MENU_DROP_MUSIC_FAIL="它拒绝离开小主您呢~~";

    public static final String MUSIC_STATUS_LOCAL="这是本地音乐哟~~";

    public static final String MUSIC_STATUS_NET="这是云音乐哟~~";
}
